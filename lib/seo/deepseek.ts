...
function getFallbackHtml(kind: HiddenKind, payload: Record<string, unknown>): string {
  ...
  switch (kind) {
    case 'product':
      return `<h1>MM Pharma — ${name} : produit d'hygiène et de désinfection professionnel</h1><p>Découvrez ${name}, un produit de qualité pharmaceutique fabriqué au Maroc par MM Pharma.</p>`;
    case 'blog':
    ...
    default:
      return `<h1>MM Pharma — ${name}</h1><p>MM Pharma, fabricant de produits d'hygiène et de désinfection au Maroc.</p>`;
  }
}
